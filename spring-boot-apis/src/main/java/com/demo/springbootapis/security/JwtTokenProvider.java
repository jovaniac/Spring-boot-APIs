...
public class JwtTokenProvider {
	...
	public String generateToken(Authentication authentication) {
		...
		return Jwts.builder().setClaims(claims).setIssuedAt(now).setExpiration(expiration)
				.signWith(SignatureAlgorithm.HS512, appProperties.getJwt().getTokenSecret()).compact();
	}

	public Long getUserIdFromJWT(String token) {
		Claims claims = Jwts.parser().setSigningKey(appProperties.getJwt().getTokenSecret()).parseClaimsJws(token)
				.getBody();
	...
	public UserPrincipal getUserPrincipalFromJWT(String token) {
		...
		// TODO: get claim and authorities and create a new UserPrincipal and
		// return.
